  ...
  return (
    <main className="grid grid-cols-4 place-items-center m-5 gap-y-5">
      ...
      {modalData ? (
        ...
        <div className='bg-white p-6 rounded-lg shadow-lg overflow-y-scroll h-5/6 w-4/6'>
          <div className='flex justify-between'>
            {
              modalData.length !== 0 ? 
              <h1>{modalData[0].characters.items[0].name}</h1> 
              :
              null
            }
          <button type='button' onClick={handleCloseModal} className='inline-block'>
          ...
          { modalData.length !== 0 ? 
            ...
            <h1 className='text-4xl text-center my-40'>No comic data for this character</h1>
          )
        }
        </div>
      </div>
